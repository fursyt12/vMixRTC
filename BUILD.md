...
Исходник — один квадратный PNG: `crates/vmixrtc/icons/source.png` (1024×1024, прозрачный фон).
Сейчас там временная иконка vMixRTC («экран» с полосами, подпись RTC и красная точка «в эфире»).

Заменить иконку во всех платформах одной командой:

```bash
cd crates/vmixrtc
cargo tauri icon icons/source.png      # нужен tauri-cli: cargo install tauri-cli --version "^2"
```

Команда перегенерирует весь набор: `32x32.png`, `64x64.png`, `128x128.png`,
`128x128@2x.png`, `icon.png`, `icon.ico` (Windows), `icon.icns` (macOS) и наборы
`icons/android`, `icons/ios` для мобильных сборок. Перечислять их в конфиге не нужно —
пути уже прописаны в `bundle.icon`, поэтому достаточно пересобрать пакеты.

Требования к исходнику: PNG, квадрат (лучше 1024×1024), прозрачный фон, без важных деталей у
краёв — macOS обрезает иконку по скруглению, а Android/Windows уменьшают её до 16–32 px.
Если рисуешь текст, делай его крупным: на 32 px и меньше мелкие детали исчезают.

Пакеты Arch и `.deb`/AppImage берут иконки из `crates/vmixrtc/icons/`
(`32x32`, `64x64`, `128x128`, `128x128@2x`, `icon.png`) — после перегенерации набор
обновится и там.
